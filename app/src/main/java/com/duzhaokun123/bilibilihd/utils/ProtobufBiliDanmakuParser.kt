...
class ProtobufBiliDanmakuParser(private val dmSegMobileReplies: Array<BiliDanmaku.DmSegMobileReply?>) : BaseDanmakuParser() {
    ...
    override fun parse(): IDanmakus {
        val danmakus = Danmakus(IDanmakus.ST_BY_TIME, false, mContext.baseComparator)
        var index = 0
                    ...
                    if (baseDanmaku != null) {
                        ...
                        if (baseDanmaku.type == BaseDanmaku.TYPE_SPECIAL && text.startsWith("[")
                            ...
                            if (textArr != null && textArr.size >= 5 && TextUtils.isEmpty(textArr[4]).not()) {
                                ...
                                mContext.mDanmakuFactory.fillTranslationData(baseDanmaku, beginX,
                                        beginY, endX, endY, translationDuration, translationStartDelay, mDispScaleX, mDispScaleY)
                                mContext.mDanmakuFactory.fillAlphaData(baseDanmaku, beginAlpha, endAlpha, alphaDuraion)
                                if (textArr.size >= 12) {
                                    // 是否有描边
                                    ...
                                    if ("" != textArr[14]) {
                                        ...
                                        if (!TextUtils.isEmpty(motionPathString)) {
                                            ...
                                            if (pointStrArray.isNotEmpty()) {
                                                ...
                                                DanmakuFactory.fillLinePathData(baseDanmaku, points, mDispScaleX,
                                                        mDispScaleY)
                                            }
                                        }
                                    }
                        ...
                        if (baseDanmaku.duration != null) {
                            ...
                            val lock: Any = danmakus.obtainSynchronizer()
                            synchronized(lock) { danmakus.addItem(baseDanmaku) }
                        }
                    }
                }
            }
